...
from tkintermapview.canvas_path import CanvasPath as CanvasPathLine


def calc_pixels(tupla_1, tupla_2):
    dist_pixels = math.sqrt(math.pow((tupla_1[0]-tupla_2[0]), 2) + math.pow((tupla_1[1]-tupla_2[1]), 2))
    return dist_pixels


class TkinterMap(tkinter.Frame):
    def __init__(self, *args,
        ...
        self.canvas.bind("<Button-2>", self.mouse_middle_click)
        self.canvas.bind("<B2-Motion>", self.mouse_middle_move)
        self.canvas.bind("<Button-3>", self.mouse_right_click)
        self.canvas.bind("<ButtonRelease-1>", self.mouse_release)
        self.bind('<Configure>', self.update_dimensions)
        self.last_mouse_down_position: Union[tuple, None] = None
        self.last_mouse_right_down_position: Union[tuple, None] = None
        ...
        self.line = None
        self.path_line = None
        self.canvas_path_lines_ids = []
        self.factor_p_to_m = 0.28
        self.dist = None
        self.first_click = False
        self.fist_mouse_right_position: Union[tuple, None] = None
        ...
        self.canvas_path_list: List[CanvasPathLine] = []
    ...
    def add_right_click_menu_command(self, label: str, command: Callable, pass_coords: bool = False, pass_pixels: bool = False) -> None:
        self.right_click_menu_commands.append({"label": label, "command": command, "pass_coords": pass_coords, "pass_pixels": pass_pixels})
    ...
    def mouse_middle_move(self, event):
        ...
        self.canvas.coords(
            self.line, self.last_mouse_right_down_position[0], self.last_mouse_right_down_position[1], event.x, event.y
        )
        ...
        self.canvas.coords(
            self.dist,
            self.last_mouse_right_down_position[0] + (event.x - self.last_mouse_right_down_position[0]) / 2,
            self.last_mouse_right_down_position[1] + (event.y - self.last_mouse_right_down_position[1]) / 2,
        )
    ...
    def mouse_right_click(self, event):
        ...
        m.add_command(label=f"{coordinate_mouse_pos[0]:.7f} {coordinate_mouse_pos[1]:.7f}", command=click_coordinates_event)
        ...
        for command in self.right_click_menu_commands:
            if command["pass_coords"] and command['pass_pixels']:
                m.add_command(label=command["label"], command=partial(command["command"], coordinate_mouse_pos, mouse_pixel))
            else:
    ...
    def set_path(self, position_list=list, **kwargs) -> CanvasPathLine:
        path = CanvasPathLine(self, position_list, **kwargs)
        self.path_line = path.draw()
        self.canvas_path_list.append(path)
        self.canvas_path_lines_ids.append(self.path_line)
        return path
    ...
    def delete(self, map_object: any):
        if isinstance(map_object, (CanvasPathLine, CanvasPositionMarker, CanvasPolygon)):
            map_object.delete()
    ...
    def mouse_move(self, event):
        ...
        x, y = event.x, event.y
        old_x = self.last_mouse_down_position[0]
        old_y = self.last_mouse_down_position[1]
        mouse_move_x = old_x - x
        mouse_move_y = old_y - y
